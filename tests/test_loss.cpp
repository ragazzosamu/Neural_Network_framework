...
TEST_CASE("Loss rejects null tensors", "[loss][exceptions]") {
    auto t = make_tensor({2}, {1.0f, 2.0f});
    std::shared_ptr<Tensor> null_tensor;

    REQUIRE_THROWS_AS(Loss::mse(null_tensor, t), std::invalid_argument);
    REQUIRE_THROWS_AS(Loss::mse(t, null_tensor), std::invalid_argument);
}
...
TEST_CASE("mse computes the correct value for known inputs", "[loss][mse]") {
    ...
    REQUIRE(Loss::mse(output, target)->item() == Catch::Approx(3.5f));
}
...
TEST_CASE("mse is zero when output equals target", "[loss][mse]") {
    ...
    REQUIRE(Loss::mse(output, target)->item() == Catch::Approx(0.0f));
}
...
TEST_CASE("mse throws on size mismatch between output and target", "[loss][mse][exceptions]") {
    ...
    REQUIRE_THROWS_AS(Loss::mse(output, target), std::invalid_argument);
}
...
TEST_CASE("mse throws on empty tensors instead of dividing by zero", "[loss][mse][exceptions]") {
    ...
    REQUIRE_THROWS_AS(Loss::mse(output, target), std::runtime_error);
}
...
TEST_CASE("cross_entropy throws on size mismatch between output and target", "[loss][cross_entropy][exceptions]") {
    ...
    REQUIRE_THROWS_AS(Loss::cross_entropy(output, target), std::invalid_argument);
}
...
TEST_CASE("cross_entropy matches the hand-computed value for a one-hot target", "[loss][cross_entropy]") {
    ...
    REQUIRE(Loss::cross_entropy(output, target)->item() == Catch::Approx(0.41702986f).epsilon(1e-4));
}
...
TEST_CASE("cross_entropy assigns lower loss to a more confident correct prediction", "[loss][cross_entropy]") {
    ...
    REQUIRE(Loss::cross_entropy(confident_output, target)->item() == Catch::Approx(0.01478325f).epsilon(1e-4));
    REQUIRE(Loss::cross_entropy(unsure_output, target)->item() == Catch::Approx(1.00194258f).epsilon(1e-4));
    REQUIRE(Loss::cross_entropy(confident_output, target)->item() < Loss::cross_entropy(unsure_output, target)->item());
}